...
def disassembler ():
    ...
    machineCode = json.loads(request.data)['code']
    binary_code = machineCode
    for i in binary_code :
        if i.isspace() : continue
        instruction = instructions_handler(i)
        if instruction[25:] == '0110011' : #R-TYPE
            instruction_result[PC] = Rtype(instruction)
        if instruction[25:] == '0000011' or instruction[25:]=='0010011' or instruction[25:]=='1100111': #I-TYPE
            instruction_result[PC] = Itype(instruction)
        if instruction[25:] == '0100011' : #S-TYPE
            instruction_result[PC] = Stype(instruction)
        if instruction[25:] == '1100011' : #SB-TYPE
            instruction_result[PC] = SBtype(instruction)
        if instruction[25:] == '0110111' or instruction[25:] == '0010111' : #U-TYPE
            instruction_result[PC] = Utype(instruction)
        if instruction[25:] == '1101111' :
            instruction_result[PC] = UJtype(instruction)
        PC+=4
    
    instructions = []
    for i in instruction_result: 
        instructions.append(str(i) + ':\t'+instruction_result[i])
    return instructions
'''
